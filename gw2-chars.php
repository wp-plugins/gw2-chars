...
function gw2chars_shortcode( $attributes ) {
	...
	$chars = $wpdb->get_results( "
		SELECT id, user_id, name, picture_file, race, profession, crafting1, crafting2, teaser
		FROM $table_name
		...
		if ($name == "") {
			...
			$wpdb->query(
				$wpdb->prepare("
					...
					( name, race, profession, crafting1, crafting2, picture_file, teaser, user_id )
					VALUES ( %s, %d, %d, %d, %d, %s, %s, %d )
				", $name, $race, $profession, $crafting1, $crafting2, $picture_file, $teaser, get_current_user_id() )
			);
		...
		if ($name == "") {
			...
			$wpdb->query(
				$wpdb->prepare("
					...
					teaser = %s
					WHERE id = %d
				", $name, $race, $profession, $crafting1, $crafting2, $picture_file, $teaser, $id)
			);
...
function gw2chars_create_database() {
	...
	add_option("gw2chars_database_version", $gwwchars_database_version);
}
